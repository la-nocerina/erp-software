...
<script type="text/javascript">
    function seleziona(id, codice, gr_um, gr_sec, vel, temp, visc){
        window.opener.scrivi_id(id, codice, gr_um, gr_sec, vel, temp, visc);
        window.close();
    }
    ...
    for($i=0;$i<$num_prodotti;$i++){
        ...
        if( count($schede_tecniche>0) ){
            ...
            for($j=0; $j<count($schede_tecniche); $j++ ){
                ...
                echo "<tr onclick=\"javascript: seleziona($prodotto->idProdotto,'$prodotto->codiceInterno',$scheda->grUmido,$scheda->grSecco,$scheda->velocita,'$scheda->tempForno',$scheda->viscosita)\">
